...
class TestSpecializedWaterfall(TestCase):
    ...
    JAN_1_6: Final[TimeSlot] = TimeSlot(
        datetime.strptime("2020-01-01T00:00:00Z", "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=pytz.utc),
        datetime.strptime("2020-01-06T00:00:00Z", "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=pytz.utc),
    )
    JAN_4_8: Final[TimeSlot] = TimeSlot(
        datetime.strptime("2020-01-04T00:00:00Z", "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=pytz.utc),
        datetime.strptime("2020-01-08T00:00:00Z", "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=pytz.utc),
    )
