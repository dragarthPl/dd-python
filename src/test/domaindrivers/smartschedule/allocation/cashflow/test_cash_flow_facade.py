...
class TestCashFlowFacade(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.events_publisher = dependency_resolver.resolve_dependency(EventsPublisher)
    ...
    def test_updating_cash_flow_emits_an_event(self) -> None:
        ...
        # when
        self.cash_flow_facade._CashFlowFacade__events_publisher = mock()
        self.cash_flow_facade.add_income_and_cost(project_id, income, cost)

        # then
        mockito.verify(self.cash_flow_facade._CashFlowFacade__events_publisher).publish(
    ...
    def is_earnings_recalculated_event(
        ...
        return (
            lambda event: event.project_id == project_id
