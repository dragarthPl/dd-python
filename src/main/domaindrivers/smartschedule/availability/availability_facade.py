...
class AvailabilityFacade:
    ...
    def create_resource_slots_with_parent_id(
        self, resource_id: ResourceAvailabilityId, parent_id: ResourceAvailabilityId, time_slot: TimeSlot
    ) -> None:
    ...
    def block(self, resource_id: ResourceAvailabilityId, time_slot: TimeSlot, requester: Owner) -> bool:
        with self.__session.begin_nested():
            to_block: ResourceGroupedAvailability = self.__find_grouped(resource_id, time_slot)
    ...
    def __block(self, requester: Owner, to_block: ResourceGroupedAvailability) -> bool:
        ...
        if result:
            return cast(
                bool, self.__availability_repository.save_checking_version_by_resource_grouped_availability(to_block)
            )
        return bool(result)
    ...
    def release(self, resource_id: ResourceAvailabilityId, time_slot: TimeSlot, requester: Owner) -> bool:
        with self.__session.begin_nested():
            to_release: ResourceGroupedAvailability = self.__find_grouped(resource_id, time_slot)
    ...
    def disable(self, resource_id: ResourceAvailabilityId, time_slot: TimeSlot, requester: Owner) -> bool:
        with self.__session.begin_nested():
            to_disable: ResourceGroupedAvailability = self.__find_grouped(resource_id, time_slot)
    ...
    def __find_grouped(self, resource_id: ResourceAvailabilityId, within: TimeSlot) -> ResourceGroupedAvailability:
    ...
    def find(self, resource_id: ResourceAvailabilityId, within: TimeSlot) -> ResourceGroupedAvailability:
        normalized: TimeSlot = Segments.normalize_to_segment_boundaries(within, SegmentInMinutes.default_segment())
        return ResourceGroupedAvailability(self.__availability_repository.load_all_within_slot(resource_id, normalized))

    def find_by_parent_id(self, parent_id: ResourceAvailabilityId, within: TimeSlot) -> ResourceGroupedAvailability:
        normalized: TimeSlot = Segments.normalize_to_segment_boundaries(within, SegmentInMinutes.default_segment())
