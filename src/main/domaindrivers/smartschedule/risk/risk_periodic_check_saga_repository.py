...
class RiskPeriodicCheckSagaRepository(Repository[RiskPeriodicCheckSaga, RiskPeriodicCheckSagaId]):  # type: ignore
    @abstractmethod
    ...
    def find_by_project_id_or_create(self, project_id: ProjectAllocationsId) -> RiskPeriodicCheckSaga:
        ...
        if not found:
            found = self.save(RiskPeriodicCheckSaga(project_id))
        return found
    ...
    def find_by_project_id_in_or_else_create(
        ...
        missing: list[RiskPeriodicCheckSaga] = list(
            map(
                lambda project_id: RiskPeriodicCheckSaga(project_id),
