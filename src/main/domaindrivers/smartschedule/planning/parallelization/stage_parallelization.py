...
class StageParallelization:
    ...
    def create_sorted_nodes_recursively(
        self,
        remaining_nodes: set[Stage],
        accumulated_sorted_nodes: ParallelStagesList
    ) -> ParallelStagesList:
        already_processed_nodes: list[Stage] = (
            reduce(lambda n, m: list(m.stages), accumulated_sorted_nodes.all, [])
        )

        nodes_without_dependencies: set[Stage] = self.with_all_dependencies_present_in(remaining_nodes,
                                                                                       already_processed_nodes)

        if len(nodes_without_dependencies) == 0:
            return accumulated_sorted_nodes

        new_sorted_nodes: ParallelStagesList = accumulated_sorted_nodes.add(
            ParallelStages(nodes_without_dependencies)
        )
        new_remaining_nodes: set[Stage] = remaining_nodes.copy()

        def node_reducer(n, m):
            n.remove(m)
            return n
    ...
    def with_all_dependencies_present_in(
        ...
        return set(
            filter(
                lambda n: contains_all(present_in, n.dependencies),
                to_check
            ),
        )
